...
Sometimes the type signature of a callback function requires a parameter, but the callback implementation doesn't _use_ the parameter. In this case, it's idiomatic to name the unused parameter `_`, which declares a [wildcard variable][wildcards] that is non-binding.
...
:::version-note
Declaring non-binding [wildcard variables][wildcards] requires a [language version][] of at least 3.7.
...
A single linter rule handles all the ordering guidelines: [directives_ordering.](/tools/linter-rules/directives_ordering)
